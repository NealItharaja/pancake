...
void pancake_proxy::observe_access(const std::string& key) {
    ...
    const double ad = ad::StatisticalTest::anderson_darling_statistic(reference_histogram_, sliding_histogram_->histogram());
    const double threshold = ad::StatisticalTest::threshold(sliding_histogram_->window_size(), 0.05);

    if (ad > threshold) {
        std::cerr << "[gradual][ad] distribution change detected: ad=" << ad << " threshold=" << threshold << "\n";

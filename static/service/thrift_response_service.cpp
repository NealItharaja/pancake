    ...
    switch (fid) {
      ...
      case 2:
        if (ftype == ::apache::thrift::protocol::T_I32) {
          xfer += iprot->readI32(op_code);
          __isset.op_code = true;
        } else xfer += iprot->skip(ftype);
        break;
...
uint32_t pancake_thrift_response_async_response_args::write(::apache::thrift::protocol::TProtocol* oprot) const {
  ...
  for (const auto& s : result) xfer += oprot->writeString(s);
...
void pancake_thrift_responseClient::async_response(const sequence_id& seq_id,
                                                   const int32_t op_code,
                                                   const std::vector<std::string>& result) {
  send_async_response(seq_id, op_code, result);
}

void pancake_thrift_responseClient::send_async_response(const sequence_id& seq_id,
                                                        const int32_t op_code,
                                                        const std::vector<std::string>& result) {
  int32_t cseqid = 0;
...
bool pancake_thrift_responseProcessor::dispatchCall(::apache::thrift::protocol::TProtocol* iprot,
...
void pancake_thrift_responseProcessor::process_async_response(
  ...
  void* ctx = nullptr;
  if (eventHandler_.get() != nullptr)
    ctx = eventHandler_->getContext("pancake_thrift_response.async_response", callContext);

  ::apache::thrift::TProcessorContextFreer freer(eventHandler_.get(), ctx,
                                                 "pancake_thrift_response.async_response");

  if (eventHandler_.get() != nullptr)
    eventHandler_->preRead(ctx, "pancake_thrift_response.async_response");
  ...
  if (eventHandler_.get() != nullptr)
    eventHandler_->postRead(ctx, "pancake_thrift_response.async_response", bytes);

  try {
    iface_->async_response(args.seq_id, args.op_code, args.result);
  } catch (...) {
    if (eventHandler_.get() != nullptr)
      eventHandler_->handlerError(ctx, "pancake_thrift_response.async_response");
  }

  if (eventHandler_.get() != nullptr)
    eventHandler_->asyncComplete(ctx, "pancake_thrift_response.async_response");
}
...
pancake_thrift_responseProcessorFactory::getProcessor(const ::apache::thrift::TConnectionInfo& connInfo) {
  ...
  return ::apache::thrift::stdcxx::shared_ptr<::apache::thrift::TProcessor>(
      new pancake_thrift_responseProcessor(handler));
}
